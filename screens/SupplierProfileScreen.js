...
export default function SupplierProfileScreen({ navigation, route }) {
  ...
  const handleEditProfile = () => navigation.navigate('EditProfile', { profileType: 'supplier' });

  const handleOpenLink = () => {
    const url = liveProfile?.supplierWebsite;
    if (!url) return;
  ...
  const name       = liveProfile?.companyName || liveProfile?.name || 'Add business name';
  const category   = liveProfile?.supplierCategory || 'Add category';
  const city       = liveProfile?.city || '';
  const state      = liveProfile?.state || '';
  const location   = [city, state].filter(Boolean).join(', ');
  const website    = liveProfile?.supplierWebsite || '';
  const about      = liveProfile?.supplierAbout || '';
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 24 }}>
        ...
        <ProfileCard>
          ...
          <View style={s.typeAvailRow}>
            <Text style={s.heroType} numberOfLines={1}>{category}</Text>
          </View>
        ...
        <ProfileCard>
          ...
          <Text style={about ? s.aboutText : s.placeholder}>
            {about || 'Add a short description to attract more buyers'}
          </Text>
        </ProfileCard>
